...
CURRENT_MODEL_DIR="VEP+TSScounts+MMSplice+expr-multitask_weighted"
...
CURRENT_MODEL_DIR=os.path.join(MODEL_DIR, CURRENT_MODEL_DIR)
...
xrds.cdf.head().values

stacked_xrds = xrds.stack(observations=["individual", "gene"])

# ## Select blood gene expression as input feature

blood = stacked_xrds.sel(subtissue="Whole Blood")
blood

# ## Load TSS counts and stack them (`observations = ["individual", "gene"]`, `features = ["zygocity", "window_index"]`)

os.path.join(PROCESSED_DATA_DIR, "OUTRIDER", "promoter_counts.zarr")

promoter_counts = xr.open_zarr(os.path.join(PROCESSED_DATA_DIR, "gtex/promoter", "promoter_counts.zarr"))

promoter_counts

promoter_counts_stacked = promoter_counts.stack(observations=["individual", "gene"], features = ["zygocity", "window_index"])
promoter_counts_stacked

# ## Load MMSplice features and stack them (`observations = ["individual", "gene"]`)

mmsplice = xr.open_zarr(os.path.join(PROCESSED_DATA_DIR, "MMSplice", "agg_gene.zarr")).rename(genes="gene")

mmsplice

mmsplice_stacked = mmsplice.stack(observations=["individual", "gene"])
mmsplice_stacked
...
vep = vep.rename({"genes":"gene"}, axis=1).set_index(["individual", "gene"])
vep.head()
...
stacked_xrds, promoter_counts_stacked, vep_xr, mmsplice_stacked = xr.align(stacked_xrds, promoter_counts_stacked, vep_xr, mmsplice_stacked, copy=False, join="left", fill_value=0.)
mmsplice_stacked
    ...
    **kwargs
    ...
    for i, da in enumerate(darrs):
        ...
        if isinstance(da, list) or isinstance(da, tuple):
            da = concat_by_axis(da, dims=dims, axis=axis + 1, **kwargs)
...
features = concat_by_axis([
    blood.log_cdf.expand_dims({"features": ["cdf"]}, axis=-1), 
    blood.zscore.expand_dims({"features": ["zscore"]}, axis=-1), 
    ...
    promoter_counts_stacked["tss_counts"],
    vep_xr,
...
stacked_xrds["zscore"] = stacked_xrds["zscore"].fillna(0)
...
y=train.zscore.transpose("observations", "subtissue", transpose_coords=False).data
y

class_weight=train.class_weight.transpose("observations", "subtissue", transpose_coords=False).data
class_weight

x, y, class_weight = dask.compute(x, y, class_weight)
...
#     y=train.zscore.transpose("observations", "subtissue", transpose_coords=False).data, 
#     class_weight=train.class_weight.transpose("observations", "subtissue", transpose_coords=False).data,
    x=train.c_features.transpose("observations", "features", transpose_coords=False).values, 
    y=train.zscore.transpose("observations", "subtissue", transpose_coords=False).values, 
    class_weight=train.class_weight.transpose("observations", "subtissue", transpose_coords=False).values,
#     batch_size=299155, 
    batch_size=100000, 
    shuffle=False,
...
# +
# model = k.models.load_model(os.path.join(CURRENT_MODEL_DIR, "model.h5"))
# -
