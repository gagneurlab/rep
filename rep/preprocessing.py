...
if __name__ == '__main__':
    ...
    genes = pd.DataFrame(index=['g1', 'g2', 'g3'])
    samples = pd.DataFrame(data={'Tissue': ['t1', 't2', 't3'],
                                 'Individual': ['i1', 'i1', 'i2']},
                           index=['s1', 's2', 's3'])

    annobj = RepAnnData(X=pd.DataFrame(data=X,index=samples.index,columns=genes.index))
    annobj.set_genes(genes)
    annobj.set_samples(samples)
